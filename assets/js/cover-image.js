...
( function( exports, $ ) {
	...
	bp.CoverImage = {
		...
		uploaderView: function() {
			...
			// Display it
			uploader.inject( '#profile-upload-cover' );
		},
		...
		uploadProgress: function() {
			...
			// Display it
			coverImageUploadProgress.inject( '#profile-upload-status' );
		},
		...
		deleteView: function() {
			...
			// Display it
			deleteView.inject( '#profile-upload-manage' );
		},
		...
		deleteCoverImage: function( model ) {
			...
			bp.ajax.post( 'bp_cover_image_delete', {
				...
				coverImageStatus.inject( '#profile-upload-status' );
				$( '.profile-cover-img' ).attr( {
					'src': jeremyCoverImages['default']
				} );
		...
		displayWarning: function( message ) {
			...
			this.warning.inject( '#profile-upload-status' );
		}
	};
	...
	bp.Views.coverImageUploadProgress = bp.Views.uploaderStatus.extend( {
		...
		uploadResult: function( model ) {
			...
			if ( ! _.isUndefined( model.get( 'url' ) ) ) {
				...
				this.views.set( '#profile-upload-status', new bp.Views.CoverImageStatus( {
					value : message,
					type  : type
				} ) );
	...
	bp.Views.DeleteCoverImage = bp.View.extend( {
		...
		id: 'profile-cover-delete',
		template: bp.template( 'profile-cover-delete' ),

		events: {
			'click #delete-cover-image': 'deleteCoverImage'
		},
