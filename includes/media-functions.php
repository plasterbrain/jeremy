...
if ( ! function_exists( 'jeremy_enqueue_media_scripts' ) ) :
/**
 * Registers, localizes, and enqueues scripts and styles related to
 * lightboxes and other media functionality.
 *
 * @since 1.0.0
 */
function jeremy_enqueue_media_scripts() {
	wp_register_script( 'swipebox', get_template_directory_uri() . '/assets/js/min/swipebox.min.js', array('jquery'), null, true );
	if ( get_theme_mod( 'use-lightbox', true ) ) {
		wp_localize_script( 'swipebox', 'jeremy', array(
			'title' => esc_html__( 'Image lightbox', 'jeremy' ),
			'loading' => esc_html__( 'Loading', 'jeremy' ),
			'close' => esc_html__( 'Close lightbox', 'jeremy' ),
			'next' => esc_html__( 'Next image', 'jeremy' ),
			'previous' => esc_html__( 'Previous image', 'jeremy' ),
		) );
		wp_enqueue_script( 'swipebox' );
	}
}
endif;
#add_action( 'wp_enqueue_scripts', 'jeremy_enqueue_media_scripts' );


if ( ! function_exists( 'jeremy_calculate_image_sizes' ) ) :
/**
 ...
 */
function jeremy_calculate_image_sizes( $output, $size ) {
...
add_filter( 'wp_calculate_image_sizes', 'jeremy_calculate_image_sizes', 10, 2 );

/**
 * Adds the classes 'entry-embed-{$provider}' and 'entry-embed-{$type}' to
 * oEmbed containers. This function is only run the first time the
 * content is fetched, so it won't apply to any cached oEmbed content.
 *
 * @since 1.0.0
 *
 ...
 */
function jeremy_oembed_parse( $return, $data ) {
	if ( is_object( $data ) ) {
		$classes = array();
		if ( ! empty( $data->type ) ) {
...
add_filter( 'oembed_dataparse', 'jeremy_oembed_parse', 0, 2 );

/**
 * Adds a generic 'entry-embed' class to embedded content containers that
 * don't already have a class that matches the 'entry-embed-{$provider}'
 * 'entry-embed-{$type}' format.
 *
 * @since 1.0.0
 *
 * @param string $cache     The cached HTML for the embedded content.
 * @return string           The cached HTML, possibly wrapped in a div
 * 							with the new class.
 */
function jeremy_oembed_wrap( $cache ) {
	if ( strpos( $cache, 'class="entry-embed' ) === false ) {
		$cache = '<div class="entry-embed">' . $cache . '</div>';
	}
	return $cache;
}
add_filter( 'embed_oembed_html', 'jeremy_oembed_wrap' );
 ...
 * 		@type int $ID			Image attachment ID. The key is
 * 								capitalized to match Meta Box's
 * 								formatting.
 ...
 * 								separated by a space. 'gallery' is added
 * 								to the start of the list automatically.
 * 		@type string $link		What the images should link to. Accepts
 * 								'file', 'none', or an empty string if you
 * 								want to link to attachment pages. That's
 * 								WP's genius idea, by the way, not mine.
 * }
 * @return string	            The gallery HTML markup.
 */
function jeremy_get_gallery( $images, $atts ) {
	$defaults = array(
		'id' => '',
		'class' => 'gallery-col-3',
		'link' => '',
...
if ( ! function_exists( 'jeremy_post_gallery' ) ) :
/**
 * Directs the native {@see gallery_shortcode} through {@see
 * jeremy_get_gallery} so that native both native WordPress galleries
 * and those created using post format meta have the same format.
 *
 * A photography theme might benefit from adding {@see
 * wp_get_attachment_metadata}, which returns detailed image metadata
 ...
 */
function jeremy_post_gallery( $output, $atts ) {
	$post = get_post();
	$atts  = shortcode_atts(
		array(
			'order'      => 'ASC',
	...
	if ( ! empty( $atts['include'] ) ) {
		...
		// User wants all the post's attachments, maybe with exclusions.
...
add_filter( 'post_gallery', 'jeremy_post_gallery', 10, 2 );
endif;

if ( ! function_exists( 'jeremy_image_send_to_editor' ) ) :
/**
 * Adds a title and rel attribute to anchor elements if an image is
 * linked to its source file. This is so {@see
 * jeremy_caption_shortcode} can turn it into a lightbox as
 * appropriate. {@see image_send_to_editor} for a description of the
 * parameters.
 *
 * @since 1.0.0
 *
 ...
 */
function jeremy_image_send_to_editor( $html, $id, $caption, $title, $align, $url, $size, $alt ) {
...
add_filter( 'image_send_to_editor', 'jeremy_image_send_to_editor', 10, 8 );
endif;

if ( ! function_exists( 'jeremy_caption_shortcode' ) ) :
/**
 * A custom version of {@see img_caption_shortcode}.
 *
 * @since 1.0.0
 */
function jeremy_caption_shortcode( $output, $attr, $content ) {
	$atts = shortcode_atts(
        array(
            'id'         => '',
            'caption_id' => '',
...
add_filter( 'img_caption_shortcode', 'jeremy_caption_shortcode', 10, 3 );
endif;

if ( ! function_exists( 'jeremy_playlist_shortcode' ) ) :
/**
 * A custom version of WordPress's playlist shortcode.
 *
 * @since 1.0.0
 */
function jeremy_playlist_shortcode( $output, $attr, $instance ) {
	global $post;
	$atts = shortcode_atts(
		array(
			'type'         => 'audio',
	...
	if ( 1 === $instance ) {
		 ...
		 * @since 3.9.0
		 *
		 * @param string $type  Type of playlist. Accepts 'audio' or
		 * 						'video'.
		 * @param string $style The 'theme' for the playlist. Core
		 * 						provides 'light' and 'dark'.
		 */
...
<figure class="entry-embed-audio wp-playlist wp-<?php echo $safe_type; ?>-playlist wp-playlist-<?php echo $safe_style; ?>">
	<div class="audio-column-right">
		...
		<<?php echo $safe_type; ?>
			controls="controls"
			preload="none"
			width="<?php echo (int) $theme_width; ?>"
...
add_filter( 'post_playlist', 'jeremy_playlist_shortcode', 10, 3 );
endif;